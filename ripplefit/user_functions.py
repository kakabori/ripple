...
def set_lattice_parameters(**kwargs):
    global rip
    for key in kwargs:
        rip.latt_par[key].value = kwargs[key]
    
def set_initial_values(**kwargs):
    global rip
    for key in kwargs:
        rip.edp_par[key].value = kwargs[key]

def set_parameters_free(**kwargs):
    global rip
    for key in kwargs:
        rip.edp_par[key].vary = kwargs[key]
...
def export_EDP_endpoints(filename="EDP.dat", start=(-10,10), end=(10,10), N=100):
    global data
    global edm
    X, Z, DIST, EDP = edm.get_EDP_endpoints(start, end, N, data)
    _export_EDP(X, Z, DIST, EDP, filename)    

def plot_EDP_endpoints(start=(-10,10), end=(10,10), N=100): 
...
def export_EDP_angle(filename="EDP.dat", center=(0,0), angle=-11.8, length=100, stepsize=0.1):
    global data
    global edm
    X, Z, DIST, EDP = edm.get_EDP_angle(center, angle, length, stepsize, data)
    _export_EDP(X, Z, DIST, EDP, filename)
     
def plot_EDP_angle(center=(0.0), angle=-11.8, length=100, stepsize=0.1):
    global data
    global edm
    X, Z, DIST, EDP = edm.get_EDP_angle(center, angle, length, stepsize, data)
    _plot_EDP(DIST, EDP) 
